...
%% Output
function [accuracyMean,accuracyStd,F_LCAMean,FHMean,TIEmean] = FS_Kflod_TopDownKNNClassifier(data, numFolds, tree, feature, numberSel,indices)
...
for k = 1:numFolds
    ...
    %% Creat sub table
    [trainDataMod, trainLabelMod] = create_SubTable2(train_data, tree);
...
%             trainData = sparse(trainDataMod{i});
            ...
            [nb{i}]  = ClassificationKNN.fit(trainData(:,selFeature),trainLabel,'NumNeighbors',5);
            end
%             [model{i}]  = train(double(trainLabel), sparse(trainData(:,selFeature(1:numberSel))), '-c 2 -s 0 -B 1 -q');
    ...
    %%           Prediction       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [predict_label] = FS_topDownKNNPrediction(test_data, nb, tree,feature,numberSel) ;
